...
import org.bytedeco.javacpp.opencv_imgproc.{circle, rectangle}
import org.bytedeco.javacpp.{DoublePointer, IntPointer, opencv_core}
...
object OpenCVUtils {
  ...
  def drawOnImage(image: Mat, points: Point2fVector): Mat = {
    ...
    for (i <- 0 until points.size.toInt) {
      val p = points.get(i)
      circle(dest, new opencv_core.Point(round(p.x), round(p.y)), radius, red)
    }
  ...
  def toPoint(p: Point2f): opencv_core.Point = new opencv_core.Point(round(p.x), round(p.y))
